...
class PDFMerger
{
	...
	{
		if(!isset($this->_files) || !is_array($this->_files)): echo("No PDFs to merge."); endif;
    ...
    $fpdi->SetPrintFooter(false);
		...
		foreach($this->_files as $file)
		{
			...
			else
			{
				...
				{
					if(!$template = $fpdi->importPage($page)): echo("Could not load page '$page' in PDF '$filename'. Check that the page exists."); endif;
					$size = $fpdi->getTemplateSize($template);
					$orientation = ($size['h'] > $size['w']) ? 'P' : 'L';
		...
		else
		{
			...
			{
				echo("Error outputting PDF to '$outputmode'.");
				return false;
			}
		}
	...
	private function _rewritepages($pages)
	{
		...
		foreach($part as $i)
		{
			...
			if(count($ind) == 2)
			{
				...
				if($x > $y): echo("Starting page, '$x' is greater than ending page '$y'."); return false; endif;
